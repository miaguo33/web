...
var lottery = {
	...
	_stop: function () {
		...
		upresult(this.options.price,this.options.target);
		$("#count").html(this.options.count);
		if(this.options.count<this.options.perCost){
			lottery._disable();
			$("#lottery").unbind();
			$("#lottery").bind("click",function(){
				alert("您的派币不够了！赶快去赚更多派币吧~");
			})
		}
	},
...
function upresult(price,result){
	var html='<img src="images/border_top.jpg" />';
		html+='<img class="result" src="images/result.png" />';
		if(price=="谢谢参与"){
			html+='<h4 class="pullCenter brown">不好意思，没抽中~</h4>';
		}else{
			html+='<h4 class="pullCenter brown">恭喜您！获得'+price+'</h4>';
		}
		html+='<button id="again" class="brown">再抽一次</button>';
		html+='<button id="more" class="brown">查看更多优惠</button>';
	layer.open({
	    content: html,
	    ...
	    success:function(elem){
			...
			$("#more").click(function(){
				location.href='http://www.higoin.com/webapp/mp_new/coin_coupons.html';
			});
	    }
	});
//	alert(result);
