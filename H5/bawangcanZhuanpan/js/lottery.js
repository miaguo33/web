...
var lottery = {
	...
	_stop: function () {
		...
		_this._enable();
		$(".td_5 img").attr('src',"images/start_light.png");
		upresult(this.options.price,this.options.resultinfo);
		if(this.options.remainchance<=0){
			lottery._disable();
			$("#lottery").unbind();
...
function upresult(price,result){
	var html='<img src="images/border_top.jpg" />';
		if(price=="谢谢参与"){
			html+='<h4 class="pullCenter brown pricename">哎呀，没抽中TAT</h4>';
		}else{
			html+='<div class="logo"><img src="images/store_info/logo/'+result.merchantCode+'.jpg"/></div>';
			html+='<h4 class="pullCenter yellow">'+result.merchantName+'</h4>';
			html+='<h4 class="pullCenter brown pricename">'+price+'</h4>';
			if(result.param1&&result.param2){
				var createTime = result.param1.split(' ');
				var endTime = result.param2.split(' ');
				html+='<p class="pullCenter brown">使用期限：'+createTime[0] + '-' + endTime[0]+'</p>';
			}
			html+='<img class="QRcode" src="images/store_info/QRcode/'+result.merchantCode+'.jpg"/>';
			html+='<p class="pullCenter black">长按二维码关注公众号注册会员领取</p>';
		}
	layer.open({
	    content: html,
